...
- **colcon** — 워크스페이스 전체를 빌드하는 빌드 도구(명령). src 아래 패키지들을
  의존성 순서대로 빌드하는 것을 담당한다. (1장에서 ros-dev-tools 로 함께 설치했다)
- **ament** — 패키지 하나하나가 어떻게 빌드되는지를 정의하는 빌드 시스템. 언어에 따라
  두 종류를 쓴다.
  - ament_python — Python 패키지용. setup.py/setup.cfg 로 설정.
  - ament_cmake — C++ 패키지용. CMakeLists.txt 로 설정.

비유하자면 colcon 은 공장 관리자(전체 작업 지시), ament 는 각 작업대의 작업 지침서다.
...
> ⏱ 첫 빌드는 패키지가 없어도 30초 안팎이 걸릴 수 있다. 커서가 멈춘 것처럼 보여도
> 기다린다. 진행 상황을 보려면 --event-handlers console_direct+ 옵션을 추가한다.
...
> ⚠️ ~/.bashrc 에 넣을 때는 반드시 절대 경로로 써야 한다.
> ```bash
...
├── setup.py            ← 빌드/설치 설정 진입점 (ament_python)
└── setup.cfg           ← 실제 설정 내용 (패키지명, 스크립트 등록 등)
```

> 📌 Jazzy(Python 3.12) 기준으로 실질적인 설정은 setup.cfg 에 담는다.
> setup.py 는 setuptools 를 호출하는 한 줄짜리 진입점으로만 남아 있다.
>
> setup.cfg 의 핵심 부분:
> ```ini
> [metadata]
...
> ```
>
> 노드 실행 명령 등록은 이 console_scripts 에서 이루어진다.
...
<package format="3">
  ...
  <depend>rclpy</depend>   <!-- 이 패키지가 rclpy 를 필요로 함 -->
</package>
```

colcon 은 이 의존성을 읽어 빌드 순서를 정한다. A가 B에 의존하면 B를 먼저 빌드한다.

### setup.cfg (Python) / CMakeLists.txt (C++) — 무엇을 빌드·설치하는가

특히 Python 에서 노드를 실행 명령으로 등록하는 곳이 setup.cfg 의 console_scripts 다.
3장에서 노드를 만들면 여기에 등록해야 ros2 run 으로 실행된다. 지금은 위치만 기억한다.

> 📌 자주 하는 실수: 노드 .py 를 만들어도 console_scripts 에 등록하지 않으면
...
- **코드 해설 포인트**: package.xml, setup.cfg, CMakeLists.txt 가 빌드와 실행에 어떻게 관여하는지 해설한다.
...
> 작성하고, 토픽(topic)으로 메시지를 발행/구독한다. setup.cfg 의 console_scripts 가
> 드디어 채워진다.
