...
> - `colcon`과 `ament`의 역할, 그리고 빌드–소싱(sourcing)의 흐름을 이해한다.
> - 내 손으로 워크스페이스를 만들고 `colcon build` 한다.
> - Python 패키지(`ament_python`)와 C++ 패키지(`ament_cmake`)를 각각 생성하고 구조를 읽는다.
> - 오버레이/언더레이(overlay/underlay) 개념으로 "내 코드가 왜 인식되는가"를 안다.
...
1장에서 ROS 2를 설치하고 `talker`/`listener`를 돌려 봤다. 그건 *남이 만든* 노드였다.
이 장부터는 **내 코드를 담을 그릇**을 만든다. 그 그릇이 워크스페이스이고, 그릇 안의
...
> 모든 소스 패키지는 워크스페이스의 **`src/` 폴더 안**에 둔다.

`src/`에 패키지를 넣고 워크스페이스 최상위에서 빌드하면, ROS 2는 자동으로 `src` 아래를
훑어 패키지들을 찾아 빌드한다. 빌드 결과는 워크스페이스 안의 세 폴더에 생긴다.
...
- **`colcon`** — 워크스페이스 전체를 빌드하는 **빌드 도구(명령)**. "src 아래 패키지들을
  의존성 순서대로 빌드하라"를 담당한다. (1장에서 `ros-dev-tools`로 함께 설치했다)
- **`ament`** — 패키지 **하나하나가 어떻게 빌드되는지**를 정의하는 빌드 시스템. 언어에 따라
  두 종류를 쓴다.
  - `ament_python` — **Python 패키지**용. `setup.py`로 설정.
  - `ament_cmake` — **C++ 패키지**용. `CMakeLists.txt`로 설정.

비유하자면 `colcon`은 *공장 관리자*(전체 작업 지시), `ament`는 *각 작업대의 작업 지침서*다.

```text
colcon build            ← 관리자: src/ 아래 모든 패키지를 빌드
   ├─ my_py_pkg   (ament_python  → setup.py 보고 빌드)
   └─ my_cpp_pkg  (ament_cmake   → CMakeLists.txt 보고 빌드)
```

> 🔁 **Foxy → Jazzy 차이**
> 빌드 도구·시스템의 큰 틀(`colcon`+`ament`)은 Foxy와 동일하다. 다만 Jazzy의 기반인
> **Ubuntu 24.04는 Python 3.12**를 쓰는데, 이 버전에서 `ament_python` 패키지를 빌드하면
> `setup.py`/`setuptools` 관련 **경고(deprecation warning)** 가 보일 수 있다. 빌드는
> 정상적으로 끝나며 동작에 문제없다 — 경고와 오류를 구분하는 눈이 필요하다(§2.8).
...
> 📌 이 줄이 바로 §2.7에서 설명할 **오버레이**를 활성화하는 명령이다. 새 터미널을 열 때마다
...
```text
my_py_pkg/
├── my_py_pkg/          ← 실제 파이썬 모듈(노드 .py를 여기에 둔다)
│   └── __init__.py
├── resource/my_py_pkg
├── test/               ← 자동 생성된 테스트(스타일 검사 등)
├── package.xml         ← 패키지 메타정보·의존성
├── setup.py            ← 빌드/설치 설정 (ament_python)
└── setup.cfg
```

빌드하고 소싱한다. (반드시 워크스페이스 **최상위**에서 빌드)
...
> 💡 Python 코드는 수정할 때마다 다시 빌드하기 번거롭다. **심볼릭 링크 설치**를 쓰면
...
├── src/                ← 소스(.cpp)를 여기에 둔다
├── package.xml         ← 메타정보·의존성 (Python과 동일 형식)
...
Python·C++ 공통이다. 패키지 이름, 버전, 설명, 라이선스, 그리고 **의존성**을 적는다.

```xml
<depend>rclpy</depend>          <!-- 이 패키지가 rclpy를 필요로 함 -->
```
...
특히 Python에서 **노드를 실행 명령으로 등록**하는 곳이 `setup.py`다. 3장에서 노드를
만들면 여기에 `entry_points`로 등록해야 `ros2 run`으로 실행된다(지금은 위치만 기억).
...
> 📌 **자주 하는 실수**: 노드 `.py`를 만들어도 `setup.py`의 `console_scripts`에 등록하지
> 않으면 `ros2 run`이 못 찾는다. "코드는 맞는데 실행이 안 될 때" 1순위로 의심할 곳이다.
...
```text
[언더레이 underlay]  source /opt/ros/jazzy/setup.bash   ← ROS 2 본체(시스템 설치)
        ▲ 그 위에 얹힘
[오버레이 overlay]   source ~/ros2_ws/install/setup.bash ← 내 워크스페이스
```

- **언더레이**: ROS 2 배포판 자체. 보통 `~/.bashrc`에서 자동 소싱(1장).
- **오버레이**: 내 워크스페이스. 언더레이 *위에* 얹혀, 같은 이름이 있으면 오버레이가 우선.

즉 `source install/setup.bash`는 "내가 방금 빌드한 패키지들을 이 셸이 알게 하라"는 뜻이다.
이걸 안 하면 `ros2 run my_py_pkg ...`가 "패키지를 못 찾음" 오류를 낸다.

> ⚠️ **흔한 함정**: *빌드한 터미널과 실행하는 터미널이 다른* 경우. 새 터미널에는 오버레이가
> 안 걸려 있다. 새 터미널에서 실행하려면 그 터미널에서 다시 `source install/setup.bash`.
...
- **버전/환경 체크**: Ubuntu 24.04의 Python 3.12와 setuptools 경고, Jazzy의 colcon 사용 흐름을 기준으로 정리한다.
...
| `ros2 run`이 패키지를 못 찾음 | 오버레이 미소싱 | 워크스페이스에서 `source install/setup.bash` |
| `src` 비었는데 빌드해도 install 없음 | 워크스페이스 최상위 아님 | `cd ~/ros2_ws` 후 `colcon build` |
| `SetuptoolsDeprecationWarning` 등 노란 경고 | Ubuntu 24.04 Python 3.12 | **경고일 뿐, 무시 가능**. 빌드 성공 여부는 마지막 `Summary` 줄로 판단 |
...
> 📌 **경고 ≠ 오류**. `colcon build` 마지막의 `Summary: N packages finished`가 보이면
...
1. `~/ros2_ws`에 `my_first_pkg`라는 **Python 패키지**를 `rclpy` 의존성으로 추가 생성하고
...
5. (생각해보기) 왜 `src/`만 git에 올리고 `build/install/log`는 올리지 않을까?
...
> **다음 장 예고** — 3장에서는 방금 만든 `my_py_pkg`/`my_cpp_pkg` 안에 **첫 노드**를
...
ls demo_py                          # demo_py/ package.xml setup.py ...
...
3장에서 이 패키지 안에 노드를 넣으면 ④의 `executables`에 노드 이름이 등장한다.
...
- **5번** `build/install/log`는 재생성 가능한 산출물이라 git에 올리면 용량만 키우고 충돌을
  부른다. **소스(src)만 버전 관리**하는 것이 원칙이다.
