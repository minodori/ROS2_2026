...
> - colcon과 ament의 역할, 그리고 빌드와 소싱(sourcing)의 흐름을 설명할 수 있다.
> - 내 손으로 워크스페이스를 만들고 colcon build 할 수 있다.
> - Python 패키지(ament_python)와 C++ 패키지(ament_cmake)를 각각 생성하고 구조를 읽을 수 있다.
> - 오버레이/언더레이(overlay/underlay) 개념으로 내 코드가 왜 인식되는지 설명할 수 있다.
