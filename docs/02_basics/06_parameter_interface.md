...
> **학습 목표**
> - 파라미터(parameter)로 노드를 재컴파일 없이 설정할 수 있다.
> - CLI와 YAML 두 가지 방법으로 파라미터를 주입하고 런타임에 변경할 수 있다.
> - Python과 C++ 양쪽에서 파라미터를 선언하고 사용할 수 있다.
> - 나만의 메시지/서비스/액션 타입(.msg/.srv/.action)을 만들고 빌드할 수 있다.
